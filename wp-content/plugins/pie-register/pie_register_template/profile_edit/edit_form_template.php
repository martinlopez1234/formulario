...
class Edit_form_template extends Edit_form
{
	...
	function addTextField(){
		...
		// '.$cl_class.'"'.$cl_data.' 
		$data  = '<div class="fieldset">'.$this->addLabel();
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.'  '.$this->addValidation().' placeholder="'.$this->field['placeholder'].'" type="text" value="'.$val.'" />';
		$data .= $this->addDesc();
	...
	function addProfilePic()
	{
		...
		$data .= '<input id="'.$this->id.'" name="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" class="'.$this->addClass().' piereg_validate[funcCall[checkExtensions],ext[gif|GIF|jpeg|JPEG|jpg|JPG|png|PNG|bmp|BMP]] re-upload-pic" '.$this->addValidation().' type="file"  />';
		$data .= '<input '.$this->read_only.' id="'.$this->id.'_hidden" name="'.$this->name.'_hidden" value="'.$val.'" type="hidden"  />';
	...
	function addTextArea(){
		...
		$data = '<div class="fieldset">'.$this->addLabel();
		$data .='<textarea '.$this->read_only.' id="'.$this->id.'" data-field_id="piereg_field_'.$this->no.'" name="'.$this->name.'" rows="'.$this->field['rows'].'" cols="'.$this->field['cols'].'"  class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.'  placeholder="'.$this->field['placeholder'].'">'.$val.'</textarea>';
		$data .= $this->addDesc();
		//$data .= $this->made_conditional_logic();
		$data .= '</div>';
		return $data;
	...
	function addDropdown(){
		...
		$data .= '<select '.$this->read_only.' '.$multiple.' id="'.$this->name.'" data-field_id="piereg_field_'.$this->no.'" name="'.$name.'" class="'.$this->addClass("").' '.$cl_class.'"'.$cl_data.' '.$this->addValidation().' >';
	...
	function addNumberField(){
		...
		$data = '<div class="fieldset">'.$this->addLabel();
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" data-field_id="piereg_field_'.$this->no.'" name="'.$this->name.'" class="'.$this->addClass().' '.$cl_class.'"'.$cl_data.' '.$this->addValidation().' placeholder="'.$this->field['placeholder'].'" min="'.$this->field['min'].'" max="'.$this->field['max'].'" type="number" value="'.$val.'" />';
		$data .= $this->addDesc();
		//$data .= $this->made_conditional_logic();
		$data .= '</div>';
		return $data;
	...
	function addPhone(){
		...
		$data = '<div class="fieldset">'.$this->addLabel();
		$data .= '<input '.$this->read_only.' id="'.$this->id.'" data-field_id="piereg_field_'.$this->no.'" name="'.$this->name.'" class="'.$this->addClass().' input_fields '.$cl_class.'"'.$cl_data.' '.$this->addValidation().' placeholder="'.((isset($this->field['placeholder']))?$this->field['placeholder']:"").'" type="text" value="'.$val.'" />';
		//$data .= $this->made_conditional_logic();
		$data .= '</div>';
		return $data;
	}
	function addList()
	{
		...
		for($a = 1 ,$c=0; $a <= $this->field['rows'] ; $a++,$c++)
		{
			...
			{
				$data .= '<input '.$this->read_only.' style="width:'.$width.'%;" type="text" name="'.$this->name.'['.$c.'][]" class="input_fields" value="'.((isset($val[$c][$b-1]))?$val[$c][$b-1]:"").'"> ';
			}	
			$data .= '</div>';
		}
		$data .= '</div>';
	...
	function addDate()
	{
		...
		else if($this->field['date_type'] == "datepicker")
		{
			...
			else
			{
				...
				$data .= '<input '.$this->read_only.' id="'.$this->id.'" name="'.$this->name.'[date][]" readonly="readonly" type="text" class="'.$this->addClass().' date_start" value="'.$val.'">';
	...
	function editProfile($user){
		...
		if( is_array($this->data) && count($this->data) > 0 )
		{
			foreach($this->data as $this->field)
			{
				...
				$profile_fields_data 	.= '<li class="fields '.$_parent.$topclass.' pageFields_'.$this->pages.' piereg_li_'.$class_x.' '.$this->not_visible.'" '.$this->field_status.'>';
