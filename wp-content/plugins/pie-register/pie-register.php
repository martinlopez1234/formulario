...
require_once(PIEREG_DIR_NAME.'/classes/app/app_api.php');
require_once(PIEREG_DIR_NAME.'/classes/app/firebase.php');
require_once(PIEREG_DIR_NAME.'/classes/app/push.php') ;
...
if( !class_exists('PieRegister') ){
	class PieRegister extends PieReg_Base{
		...
		function pie_main(){
			...
			add_action( 'admin_notices', array( $this, 'pie_promo_notice_for_both' ), 1 );
			...
			add_action( 'wp_ajax_dismiss_pie_promo_notice_for_both', array( $this, 'dismiss_pie_promo_notice_for_both' ) );
		...
		*/
				...
				if( intval($id) != 0 )
				{
					...
					if($check_form_in_db == false || trim($check_form_in_db) == "")
					{
						...
						if( ($check_form_in_db['Status'] == "enable" && (!isset($check_form_in_db['IsDeleted']) || trim($check_form_in_db['IsDeleted']) != 1) ) || $is_preview){
							if (is_user_logged_in() && !$is_preview && !$this->is_pr_preview ) {
								$global_options = $this->get_pr_global_options();
									
								return apply_filters("pie_reg_form_logged_in_msg",sprintf(__("Already logged in, Click <a href='%s'>here</a> to edit profile.","pie-register"),$this->get_page_uri($global_options["alternate_profilepage"],"edit_user=1")));
							} 
							if(!$is_preview){
		...
		function add_action_links( $links, $file ) 
		{
			...
			if(!$this->piereg_pro_is_activate) $links[] = '<a style="color:#13ad11;" target="_blank" href="https://pieregister.com/plan-and-pricing/">'.__("Go Pro","pie-register").'</a>';
		...
		*/
		...
		/* 
			...
			@TODO remove this notice in the future.
		*/
		public function pie_promo_notice_for_both()
		{
			$show_release_notice   = get_option( 'pie_promo_notice_for_both_fv_addon' );
			$addon_url = "https://pieregister.com/addons/field-visibility-addon/?utm_source=plugin-freeversion&utm_medium=adminarea&utm_campaign=buy_addons&utm_content=fieldaddons";
			if($this->piereg_pro_is_activate)
			{
				$addon_url = "https://pieregister.com/addons/field-visibility-addon/?utm_source=plugin-paidversion&utm_medium=adminarea&utm_campaign=buy_addons&utm_content=fieldaddons";			
			}
			if ( empty( $show_release_notice ) ) {
				
				?>
                <div class="notice notice-info pie-admin-notice for_both is-dismissible">
					<h3><?php echo ( _e( 'Pie Register - Field visibility Addon','pireg')); ?></h3>
                	...
                	<p><a href="https://pieregister.com/addons/field-visibility-addon/?utm_source=plugin-paidversion&utm_medium=adminarea&utm_campaign=buy_addons&utm_content=fieldaddons" target="_blank" id="pie_opt_in_start" class="button button-primary"><?php _e( 'Click here for details', 'piereg' ) ?></a>&nbsp;<a href="javascript:void(0);" class="button-secondary pie_opt_in_link"><?php _e( 'No thanks', 'piereg' ) ?></a></p>
                </div>
                <script type="text/javascript">
					jQuery(document).on('click', '#pie_opt_in_start', function (e) {
						jQuery(this).parents('.for_both').find( '.notice-dismiss' ).trigger('click');
					});
                	...
                	jQuery( '.for_both' ).on( 'click', '.notice-dismiss', function() {		
						var data = {
							action: 'dismiss_pie_promo_notice_for_both'
						};
						jQuery.post( ajaxurl, data );
					});
                </script>
		...
		// @TODO remove this notice in the future.
		public function dismiss_pie_promo_notice_for_both()
		{
			update_option( 'pie_promo_notice_for_both_fv_addon', 'yes' );
		}		
...
?>
		...
		function checkLogin(){
			...
			if( (isset($_POST['piereg_login_form_nonce']) && wp_verify_nonce( $_POST['piereg_login_form_nonce'], 'piereg_wp_login_form_nonce' )) || (isset($_POST['social_site']) && $_POST['social_site'] == "true") || (isset($_POST['piereg_login_after_registration']) && $_POST['piereg_login_after_registration'] == true) )
			{
				...
				else
				{
					...
					if( $this->piereg_pro_is_activate ){
						if($this->piereg_bundle_activate) {
							do_action('pie_user_control_on_login', $this->pie_post_array, $errors);
						}
						else{
							...
							if( isset($option['piereg_blk_email']) && (isset($option['enable_blockedemail']) && $option['enable_blockedemail'] == 1 ) )
							{
								...
								if(	$this->isEmailAddressIsBlocked($cred_userlogin2,$array_emailaddr) ) {
									$errors->add('login-error',apply_filters("piereg_email_blocked",__('This email address has been blocked by administrator.','pie-register')));
									$error_found++;
								}	
							}

							/**
							* Allowed Specific Usesrs 
							* Since v3.4.7
							*/
							if( isset($option['piereg_ald_username']) && (isset($option['enable_allowedusername']) && $option['enable_allowedusername'] == 1 ) )
							{
								$cred_userlogin	= $this->pie_post_array['log'];
								if( is_email($cred_userlogin) ) {
									$userdata 			= get_user_by('email', $cred_userlogin);
								...
								if(	$this->isUserNameIsAllowed($cred_userlogin,$array_username) ) {
									$errors->add('login-error',apply_filters("piereg_username_allowed",__('This user has not been allowed by administrator.','pie-register')));
									$error_found++;
								}	
							}
							
							if( isset($option['piereg_ald_email']) && (isset($option['enable_allowedemail']) && $option['enable_allowedemail'] == 1 ) )
							{
								$cred_userlogin2	= $this->pie_post_array['log'];
								if( !is_email($cred_userlogin2) ) {
									$userdata 			= get_user_by('login', $cred_userlogin2);
								...
								$array_emailaddr 	= array_map( 'trim', explode(PHP_EOL,$option['piereg_ald_email']) );
								$array_emailaddr 	= array_map( 'strtolower', $array_emailaddr );
								
								if(	$this->isEmailAddressIsAllowed($cred_userlogin2,$array_emailaddr) ) {
									$errors->add('login-error',apply_filters("piereg_email_allowed",__('This email address has not been allowed by administrator.','pie-register')));
									$error_found++;
								}	
							}
						}											
					...
					if( sizeof($errors->errors) == 0 && $error_found == 0){
						...
						if ( is_wp_error($user))
						{
							...
							{
								$user_login_error = apply_filters('pie_invalid_user_password_msg_txt','<strong>'.ucwords(__("error","pie-register")).'</strong>: '.__("Invalid username or password","pie-register").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","pie-register").'">'.__("Lost your password?","pie-register").'</a>');
							}
		...
		function PieRegInvitationCodes()
		{
			...
			if( isset($_POST['piereg_invitation_code_nonce']) && wp_verify_nonce( $_POST['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ) )
			{
				...
				else if( isset($this->pie_post_array['send_invite_email'])  &&  $this->piereg_pro_is_activate) 
				{
					$this->piereg_get_wp_plugable_file(true); // require_once pluggable.php
					if(isset($this->pie_post_array['piereg_invitation_code_nonce']) && wp_verify_nonce( $this->pie_post_array['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ))
					{

						if(!isset($this->pie_post_array['pie_email_invitecode']) || empty($this->pie_post_array['pie_email_invitecode'])){
							$this->pie_post_array['error']['error_invite_code'] = __("Please select invitation code","pie-register");
						}
							...
							if(!empty($from_email) && filter_var($from_email,FILTER_VALIDATE_EMAIL)){ 
								...
								foreach($to_emails as $email){
									...
									if( get_user_by('email', $email) ){
										$this->pie_post_array['error'][] = sprintf(__("%s user already exists.","pie-register"),$email);
										continue;
									}
									
									$key = base64_encode($email."|".$this->pie_post_array['pie_email_invitecode']);
									$pageURL 	= add_query_arg(array('action'=>'pie-ic','key'=>urlencode($key)),get_page_link($this->pie_post_array['pie_email_linkpage']));

									$emailBody  = $piereg['pie_email_content'];
									$emailBody 	= str_replace('%invitation_link%',$pageURL,$emailBody);
									$emailBody 	= str_replace('%blogname%',get_bloginfo('name'),$emailBody);
									
									if(wp_mail($email, $subject, nl2br($emailBody), $headers)){
										$this->pie_post_array['notice'][] = sprintf(__("Invitation code successfully sent to %s","pie-register"), $email);
										// email count
										global $wpdb;
										$prefix = $wpdb->prefix."pieregister_";
										...
										$code_id = $wpdb->get_results("SELECT id FROM ".$invite_code_table_name." WHERE name='".$piereg['pie_email_invitecode']."'");
										...
										$this->pie_post_array['error'][] = sprintf(__("Failed to send invitation code to  %s","pie-register"), $email);							
										$this->pr_error_log("'The e-mail could not be sent. Possible reason: mail() function may have disabled by your host.'".($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));								
									}
								}
							}
						...
						if(isset($this->pie_post_array['add_code'])){
							...
							if($this->piereg_pro_is_activate){
								if(empty($this->pie_post_array['invitation_code_prefix']) && !empty($this->pie_post_array['invitation_code_numbers'])){
									$this->pie_post_array['error'] = __("Code Prefix is empty.","pie-register");
								}
								if(empty($this->pie_post_array['invitation_code_prefix']) && empty($this->pie_post_array['invitation_code_numbers'])){
							...
							if($update['codepass'] != ""){
								...
								if($this->piereg_pro_is_activate){
									$invite_length = (isset($this->pie_post_array['invitation_code_numbers'])) ? $this->pie_post_array['invitation_code_numbers'] : "";
									$invite_prefix = (isset($this->pie_post_array['invitation_code_prefix'])) ? $this->pie_post_array['invitation_code_prefix'] : "";
									if($invite_prefix != ''){
		...
		function SaveSettings()
		{
			...
			if( isset($_POST['action']) && $_POST['action'] == 'pie_reg_update' )
			{
				...
				else if( isset($_POST['piereg_allowed_users']) && wp_verify_nonce( $_POST['piereg_allowed_users'], 'piereg_wp_allowed_users' ) )
				{
					$this->pie_post_array	= $this->piereg_sanitize_post_data( 'piereg_allowed_users',  ( (isset($_POST) && !empty($_POST))?$_POST : array() ) );
					
					$this->piereg_get_wp_plugable_file(true); // require_once pluggable.php
					if( isset( $this->pie_post_array['allowed_user_by_username'] ) )
					{
						$update['enable_allowedusername'] 	= isset($this->pie_post_array['enable_allowedusername']) ? intval($this->pie_post_array['enable_allowedusername']) : ""; 
						$update['piereg_ald_username'] 		= $this->disable_magic_quotes_gpc($this->pie_post_array['piereg_ald_username']);
					}
					else if( isset( $this->pie_post_array['allowed_user_by_email'] ) )
					{
						$update['enable_allowedemail'] 	=  isset($this->pie_post_array['enable_allowedemail']) ? intval($this->pie_post_array['enable_allowedemail']) : ""; 
						$update['piereg_ald_email'] 	= $this->disable_magic_quotes_gpc($this->pie_post_array['piereg_ald_email']);
					}
					else
					{
						$this->pie_post_array['error'] = __("Wrong nonce","pie-register");
		...
		*/
		...
		function showLoginForm()
		{
			...
			if(isset($_GET['pr_renew_account']) && $_GET['pr_renew_account'] == true)
			{
				...
				if (is_user_logged_in() && !$this->is_pr_preview) {											
					return apply_filters("pie_login_form_logged_in_msg",__("You are already logged in.","pie-register"));
				} 
		...
		function showForgotPasswordForm()
		{
			...
			{
				return apply_filters("pie_forgot_form_logged_in_msg",__("You are already logged in.","pie-register"));
			}
			else
			{							
				$this->set_pr_stats("forgot","view");
				...
				if(trim($_POST['data']['id']) != "" and trim($_POST['data']['value']) != "" and trim($_POST['data']['type']) != "")
				{
					...
					else if(trim($_POST['data']['type']) == "expiry_date" && $this->piereg_pro_is_activate)
					{
						if($_POST['data']['value'] != ''){
							...
							$today = date('Y-m-d');
							$expiry_date = (strtotime($expiry_date) < strtotime($today)) ? '' : $expiry_date; 
							if(!empty($expiry_date)) $sql = "UPDATE `{$codetable}` SET `expiry_date`=%s WHERE `id` = ".((int)$_POST['data']['id'])."";
						}
					}
		...
		*/
		...
		function export_default_options($is_download = true){
			...
			if(isset($_POST['piereg_export_general_settings']) && wp_verify_nonce( $_POST['piereg_export_general_settings'], 'piereg_wp_export_general_settings' ))
			{
				...
				$result_redirect_settings = $wpdb->get_results(  $sql  ); #WPS
				if(isset($wpdb->last_error) && !empty($wpdb->last_error)){
					$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}
		...
		function piereg_plugin_row_meta( $links, $file ) {
			if ( $file == PIEREG_PLUGIN_BASENAME ) {
				$row_meta = array(
					...
					'support'	=>	'<a href="' . esc_url( apply_filters( 'pieregister_support_url', 'https://wordpress.org/support/plugin/pie-register/' ) ) . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pie-register' ) ) . '" target="_blank">' . __( 'Support', 'pie-register' ) . '</a>',
				);				
		...
		public function pieregister_download_user_files()
		{
					...
					foreach( $users as $user ) 
					{
						...
						if ( !empty($uploads) )
						{
							foreach( $uploads as $upload )
							{
								...
								{
									$files[$upload->user_id] = $upload->meta_value;
								}
							}
						}
					}
			...
			else
			{
				...
				foreach ( $files as $key=>$file )  
				{
					...
					$zip->addFile(wp_upload_dir()['basedir']."/piereg_users_files/{$key}/".basename($file), basename($file));
				}
