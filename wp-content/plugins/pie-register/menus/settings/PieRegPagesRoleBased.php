...
<fieldset class="piereg_fieldset_area" <?php echo $_disable; ?>>
  <?php if((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['pie_id']) && $_GET['pie_id'] != "") && $this->piereg_pro_is_activate ){ ?>
      <legend><?php echo __("Edit Record",'pie-register') . $_available_in_pro ?> </legend>
  <?php }else{ ?>
  ...
  <form method="post" id="redirect_form">
    ...
    <?php
		...
		$is_add_new = true;
		if((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['pie_id']) && $_GET['pie_id'] != "") && $this->piereg_pro_is_activate )
		{
			global $wpdb;
			...
			$is_add_new = false;
			$input_user_role 	= ((isset($this->pie_post_array['piereg_user_role']))?$this->pie_post_array['piereg_user_role']:$result[0]->user_role);
			$input_logged_in 	= ((isset($this->pie_post_array['logged_in_url']))?$this->pie_post_array['logged_in_url']:$result[0]->logged_in_url);
			$logged_in_page_id 	= ((isset($this->pie_post_array['log_in_page']))?$this->pie_post_array['log_in_page']:$result[0]->logged_in_page_id);
			$input_logout 		= ((isset($this->pie_post_array['log_out_url']))?$this->pie_post_array['log_out_url']:$result[0]->log_out_url);
			$log_out_page_id 	= ((isset($this->pie_post_array['log_out_page']))?$this->pie_post_array['log_out_page']:$result[0]->log_out_page_id);			
		}
...
<?php if( $this->piereg_pro_is_activate ) { ?>
  ...
  <div style="clear:both;float:left;padding-right:5px;margin-right:5px;">
    <form method="post" id="form_invitation_code_per_page_items">
      <?php _e("Per-Page Item","pie-register"); ?>
      <select name="invitation_code_per_page_items" id="invitation_code_per_page_items" title="<?php _e("Select Per-Page User Role Redirection","pie-register"); ?>">
        <?php
