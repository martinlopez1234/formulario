...
class Pie_api extends PieReg_Base {
    ...
    function pie_login_callback(  $request_data )
    {
      ...
      $results = $wpdb->get_results( 
                    $wpdb->prepare("
                              ...
                              WHERE
                              um1.meta_value like '%administrator%' AND um1.meta_key = 'wp_capabilities'
      ...
      if (count($results)> 0){
        ...
        $login      = isset($_POST['log']) && $_POST['log'] != '' ? $_POST['log'] : '';
    ...
    */
    ...
    function getSignInDetails($user){
      ...
      $error_code       =  isset($user->data) && $user->data != '' ? "success" : "User name/email or password is incorrect";
