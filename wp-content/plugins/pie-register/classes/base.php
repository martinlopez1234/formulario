	...
	class PieReg_Base extends PieRegisterBaseVariables
	{
		...
		*/
		...
		if($this->piereg_pro_is_activate)
		{
			...
			$structure['upload']	.= '<div class="advance_fields"><label for="file_size_%d%">'.__("File Size","pie-register").'</label><input type="number" class="input_fields" disabled><a class="info" href="javascript:;" style="color:red;">'.__("Available in premium","pie-register").'</a></div>';
		}
...
// pie-register-woocommerce Addon
		...
		function uninstall_settings()
		{
			...
			# Delete all notices
			delete_option('pie_pro_release_notice');
			delete_option('pie_admin_footer_text_rated');
			delete_option('pie_promo_notice_for_pro');
			delete_option('pie_pro_active_guide');
			delete_option('pie_review_request_delete');
			delete_option('pie_logged_pro_not_used');
			...
			// email count
			$invite_code_emails_table_name = $wpdb->prefix."pieregister_invite_code_emails";
			if(!$wpdb->query("DROP TABLE IF EXISTS `".$invite_code_emails_table_name."`")){
				$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
			}
			// ver 3.5.4
			$custom_role_table_name = $wpdb->prefix."pieregister_custom_user_roles";
			if(!$wpdb->query("DROP TABLE IF EXISTS `".$custom_role_table_name."`")){
				$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
			}
		...
		*/
		...
		public function pie_premium_check()
		{
			$options 	= get_option( PIEREG_LICENSE_KEY_OPTION );
			$_pie_premium_addon_user = get_option('pie_premium_addon_active');
			$response 	= false;
			
			if($this->piereg_pro_is_activate && is_plugin_active('pie-register-premium-features/pie-register-premium-features.php') && !$_pie_premium_addon_user)
			{
				$platform 	= site_url();
				$target_url = add_query_arg( ['wc-api'=>'am-software-api','request'=>'add_premium_user','platform'=>urlencode($platform),'activation_email'=>$options['activation_email']], $this->upgrade_url );
				...
				{
					update_option('pie_premium_addon_active','yes');
				}
			}
		...
		public function authenticate_license()
		{
			...
			$pie_pro_logged = get_option( 'pie_logged_pro_not_used' );
			$response 	= false;
			
			if(
				($this->piereg_pro_is_activate == true || $activated == "Activated") && !$pie_pro_logged &&		
				(
					!isset($options['api_key'], $options['activation_email']) || 
					(empty($options['api_key']) || empty($options['activation_email']))
				)
			)
			{
				$platform 	= site_url();
		...
		static function static_piereg_logout_url($url,$redirect="")
		{
			...
			/*
				Get after Log Out url by current user role
			*/
			if( self::piereg_pro_is_activate ) {
				global $wpdb,$current_user;
				$piereg_table_name=$wpdb->prefix."pieregister_redirect_settings";
				$current_user = wp_get_current_user();
				$current_user_roles = "'".implode("','",$current_user->roles)."'";
		...
		*/		
		...
		function anyAddonActivated(){
			...
			if(
				...
				&& 
				!is_plugin_active('pie-regiser-mailchimp/pie-regiser-mailchimp.php')
				&& 
		...
		# - SEARCH USERS BY PR FORMS IN WP USERS TABLE (START)
			...
			// ver 3.5.4
			public function pie_filter_by_invite_code($which)
			{
				$searched_form	= $this->get_click_pie_filter_users_button();
				
				// template for filtering
				$html_code 	 = '<select name="pie_invite_code_%s" style="float:none;margin-left:10px;">
								<option value="">%s</option>%s</select>';
				$options	 = "";
				...
				global $wpdb;
				$codetable    = $wpdb->prefix."pieregister_code";
				$inviteCodes  = $wpdb->get_results("SELECT `id`,`name` FROM $codetable");
				if(!empty($inviteCodes)){
					foreach($inviteCodes as $code_detail){
						$selected		= (isset($searched_form) && $code_detail->name == $searched_form )? "selected" : "";
						
						$options		.= '<option '.$selected.' value="'.$code_detail->name.'">'.$code_detail->name.'</option>';
					}
				}
								
				// combine template and options
				$select = sprintf( $html_code, $which, __( 'Invitation Code' ), $options );
			
				// output <select> and submit button
				echo $select;
				
				submit_button(__( 'Filter' ), null, 'invited_'.$which, false);
			}
			
			
			public function pie_filter_users_by_invitation_code($query)
			{
				global $pagenow;
				if (is_admin() && 'users.php' == $pagenow) {
			...
			public function get_click_pie_filter_users_button()
			{
				...
				$invitation_top    = isset($_GET['pie_invite_code_top']) 	? ($_GET['pie_invite_code_top']) 	: "";
				$invitation_bottom = isset($_GET['pie_invite_code_bottom']) 	? ($_GET['pie_invite_code_bottom']) 	: "";

				if (!empty($top) || !empty($bottom) || !empty($invitation_top) || !empty($invitation_bottom))
				{
					...
					if(isset($_GET['invited_top']) && strtolower($_GET['invited_top']) == 'filter')
						$searched_form = !empty($invitation_top) ? $invitation_top : "";

					if(isset($_GET['invited_bottom']) && strtolower($_GET['invited_bottom']) == 'filter')
						$searched_form = !empty($invitation_bottom) ? $invitation_bottom : "";
				}
