...
class Profile_admin extends PieReg_Base
{
    ...
    function addCheckRadio($disabled)
    {
        if (sizeof($this->field['value']) > 0) {
            ...
            for ($a = 0; $a < sizeof($this->field['value']); $a++) {
                ...
                echo '<input' . $disabled . ' style="margin-left:5px;" value="' . $this->field['value'][$a] . '" ' . $checked . ' type="' . $this->type . '" name="' . $this->slug . '[]" class="' . $this->field['css'] . '"  >';
            }
        }
    }
    ...
    function addPhone($disabled)
    {
		...
		echo '<input'.$disabled.' id="' . $this->id . '"  name="' . $this->slug . '" class="input_fields"  placeholder="' . (isset($this->field['placeholder']) ? $this->field['placeholder'] : "") . '" type="text" value="' . $val . '" />';
    }
    ...
    function addInvitation($disabled)
    {
		...
		echo '<input'.$disabled.' id="' . $this->id . '"  name="' . $this->slug . '" class="input_fields"  placeholder="' . (isset($this->field['placeholder']) ? $this->field['placeholder'] : "") . '" type="text" disabled="disabled" style="width:25em;" value="' . $val . '" />';
    }
			...
			*/
            ...
            if ((!isset($field_name) || empty($field_name)) && $required) {
                ...
                if (!is_numeric($field_name)) {
                    $errors->add( $this->slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'] .apply_filters("piereg_field_must_contain_only_numbers",__(' field must contain only numbers.','pie-register' )));
			    }
            } else if ($rule == "alphanumeric") {
                if (!preg_match("/^([a-z 0-9])+$/i", $field_name)) {
                   	$errors->add( $this->slug , '<strong>'.ucwords(__('error','pie-register')).'</strong>: '.$this->field['label'] .apply_filters("piereg_field_may_only_contain_alpha_numeric_characters",__(' field may only contain alpha-numeric characters.','pie-register' )));
                }
            } else if ($rule == "email") {
