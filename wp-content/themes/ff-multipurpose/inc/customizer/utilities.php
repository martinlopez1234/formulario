 ...
 */
class FF_Multipurpose_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'ff_multipurpose_customizer_defaults', array() );
	...
	public static function get_posts_as_array( $args ) {
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					$title = sprintf( esc_html__( 'ID: %s No Title', 'ff-multipurpose' ) , $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			...
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'ff-multipurpose' ),
			'entire-site'  => esc_html__( 'Entire Site', 'ff-multipurpose' ),
		);

		return apply_filters( 'ff_multipurpose_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$ff_multipurpose_utilities = new FF_Multipurpose_Customizer_Utilities();
