/* global ffMultipurposeScreenReaderText */
/**
...
(function($) {
    ...
    function initMainNavigation(container) {
        ...
        var dropdownToggle = $('<button />', {
            ...
            .append(ffMultipurposeScreenReaderText.icon)
            .append($('<span />', {
                'class': 'screen-reader-text',
                text: ffMultipurposeScreenReaderText.expand
            }));
        ...
        container.find('.current-menu-ancestor > button')
            ...
            .text(ffMultipurposeScreenReaderText.collapse);
        ...
        container.find('.dropdown-toggle').click(function(e) {
            ...
            // jscs:enable
            screenReaderSpan.text(screenReaderSpan.text() === ffMultipurposeScreenReaderText.expand ? ffMultipurposeScreenReaderText.collapse : ffMultipurposeScreenReaderText.expand);
        });
    }
    ...
    (function() {
        ...
        menuToggle.on('click.ffMultipurpose', function() {
    ...
    (function() {
        ...
        function toggleFocusClassTouchScreen() {
            if (window.innerWidth >= 910) {
                $(document.body).on('touchstart.ffMultipurpose', function(e) {
                    if (!$(e.target).closest('.main-navigation li').length) {
                        $('.main-navigation li').removeClass('focus');
                    }
                });
                siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.ffMultipurpose', function( e ) {
                ...
                siteNavigation.find('.menu-item-has-children > a').unbind('touchstart.ffMultipurpose');
            }
        }

        if ('ontouchstart' in window) {
            $(window).on('resize.ffMultipurpose', toggleFocusClassTouchScreen);
            toggleFocusClassTouchScreen();
        }

        siteNavigation.find('a').on('focus.ffMultipurpose blur.ffMultipurpose', function() {
  ...
  $(".portfolio-item:even").addClass("even-item");
    $(document).ready(function() {
        $(window)
            .on('load.ffMultipurpose resize.ffMultipurpose', function() {
                if (window.innerWidth < 910) {
